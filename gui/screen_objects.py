...
class InfoPanel(TextField):
    ...
    def slide_panel_in(self) -> None:
        ...
        visible_area_top: int = self.screen_rect.top + self.bg_rect.bottom
        visible_area_bottom: int = self.screen_rect.bottom - self.bg_rect.top
        visible_area_left: int = self.screen_rect.left + self.bg_rect.right
        visible_area_right: int = self.screen_rect.right - self.bg_rect.left
        ...
        if mode == "trigger":
            ...
            if not self.cooldown and self.duration_timer <= 0:
                if random.random() < self.chance_per_second:
                    ...
                    self.cooldown_timer: int = int(settings.frame_rate * self.cooldown_seconds)  # Set cooldown timer.
            ...
            if self.duration_timer <= 0:
                self.speed: int = self.speed_backup  # Restore the original speed.
            ...
            if self.cooldown:
                ...
                if self.cooldown_timer <= 0:
                    self.cooldown: bool = False  # Reset cooldown state when the timer ends.
